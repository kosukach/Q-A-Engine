defmodule QaEngine.PostsTest do
  use QaEngine.DataCase
  ...
  describe "questions" do
    ...
    test "update_question/2 with valid data updates the question" do
      question = question_fixture()
      assert {:ok, %Question{} = question} = Posts.update_question(question, @update_attrs)
  ...
  describe "answers" do
    ...
    test "list_answers/0 returns all answers" do
      ...
      assert Posts.list_answers() == [answer]
    end

    test "get_answer!/1 returns the answer with given id" do
      answer = answer_fixture()
      assert Posts.get_answer!(answer.id) == answer
    end

    test "create_answer/1 with valid data creates a answer" do
      assert {:ok, %Answer{} = answer} = Posts.create_answer(@valid_attrs)
      assert answer.body == "some body"
      assert answer.title == "some title"
    end

    test "create_answer/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Posts.create_answer(@invalid_attrs)
    end

    test "update_answer/2 with valid data updates the answer" do
      answer = answer_fixture()
      assert {:ok, %Answer{} = answer} = Posts.update_answer(answer, @update_attrs)
    ...
    test "update_answer/2 with invalid data returns error changeset" do
      answer = answer_fixture()
      assert {:error, %Ecto.Changeset{}} = Posts.update_answer(answer, @invalid_attrs)
      assert answer == Posts.get_answer!(answer.id)
    end

    test "delete_answer/1 deletes the answer" do
      answer = answer_fixture()
      assert {:ok, %Answer{}} = Posts.delete_answer(answer)
      assert_raise Ecto.NoResultsError, fn -> Posts.get_answer!(answer.id) end
    end

    test "change_answer/1 returns a answer changeset" do
      answer = answer_fixture()
      assert %Ecto.Changeset{} = Posts.change_answer(answer)
    end
  end
end
